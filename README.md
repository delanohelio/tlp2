...
| 12     | 11/08 e 12/08  | **Introdução ao Ambiente Moderno e Componentes**<br/>- O que são Componentes?<br/>- Single File Components (.vue)<br/>- Visão geral de `npm` e `Vite` | **Meu Primeiro Projeto com Vite:** Criar um projeto Vue e refatorar a Calculadora de IMC para usar um componente.                                                                              |
| 13     | 18/08 e 19/08  | **Comunicação entre Componentes (Pai para Filho)**<br/>- `Props`: passando dados de forma dinâmica.<br/>- Validação de `props`.                       | **Catálogo de Filmes (Parte 1):** Criar componentes `FilmeCard` e `FilmeLista`. A lista (pai) passa os dados de cada filme para o card (filho) via `props`.                                    |
